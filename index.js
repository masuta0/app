  ...
  new SlashCommandBuilder()
    ...
    .addIntegerOption(option =>
      option
        ...
        .setRequired(false)
        .addChoices(
          { name: '最速 (0.2秒)', value: 0 }, // 特別扱い → 実際は0.2秒
          { name: '1秒', value: 1 },
          { name: '5秒', value: 5 },
          { name: '10秒', value: 10 },
          { name: '15秒', value: 15 },
...
client.on(Events.InteractionCreate, async interaction => {
  ...
  if (interaction.isChatInputCommand()) {
    ...
    const cooldown = interaction.options.getInteger("cooldown") ?? 0.2;

    let messageText = "";
    let buttonId = "";

    if (commandName === "spam1") {
      messageText = "# Raid by Masumani\nこのサーバーはますまに共栄圏に荒らされました\n https://discord.gg/msmn\nMASUMANI ON TOP";
      buttonId = "spam_btn_1";
    ...
    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId(`${buttonId}_${mentionType}_${cooldown}`)
        .setLabel("実行")
        .setStyle(ButtonStyle.Primary),
    );

    await interaction.reply({
      content: `実行「${commandName}」\nメンション: ${mentionType}, クールタイム: ${cooldown}s`,
      components: [row],
  ...
  if (interaction.isButton()) {
    if (interaction.customId.startsWith("spam_btn_")) {
      ...
      const mentionType = parts[3] || "none";
      const rawCooldown = parseFloat(parts[4]);
      const cooldownValue = rawCooldown > 0 ? rawCooldown : 0.2;
      const interval = cooldownValue * 1000 + 100;
